...
function renderDashboard(){
  ...
  const reqCounts = {};
  DB.requests.forEach(r=>{ const k = r.receivedBy || 'Sin responsable'; reqCounts[k] = (reqCounts[k]||0)+1; });
  ...
  const stack = `<div class="hp-stack">
    ...
    <section class="hp hp-req">
      ${hudPanelHead('Pedidos por responsable', DB.requests.length, "goRoute('requests')")}
      <div class="hp-fit">${reqItems.length ? hudBars(reqItems) : hudEmpty('Sin pedidos cargados.')}</div>
    </section>
  </div>`;
  ...
  DB.requests.forEach(r=>feed.push({ tag:'PED', title:r.description, key:loadOrderKey(r), go:"goRoute('requests')" }));
