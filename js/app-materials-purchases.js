...
function deleteMaterial(projectId, materialId){
  ...
  ensurePurchases().forEach(pu=>{ if(pu.materialId===materialId) pu.materialId = null; });
...
function purchaseMaterialName(pu){
  if(!pu.materialId) return pu.materialText || '—';
  const p = DB.projects.find(x=>x.id===pu.projectId);
  const m = p ? ensureMaterials(p).find(x=>x.id===pu.materialId) : null;
  return m ? m.type : (pu.materialText || '—');
}
...
function purchaseMaterialFieldHtml(projectId, pu){
  ...
  return `<label class="field light"><span>Material (del cómputo del proyecto)</span>
    <select id="pu-material-id">
      <option value="">Sin vincular a un material</option>
      ${mats.map(m=>`<option value="${m.id}" ${pu?.materialId===m.id?'selected':''}>${esc(m.type)} — ${esc(String(m.qty))} ${esc(m.unit||'')}</option>`).join('')}
    </select>
  </label>`;
}
...
function savePurchase(purchaseId){
  ...
  const matIdEl = document.getElementById('pu-material-id');
  ...
  const data = {
    projectId,
    materialId: matIdEl ? (matIdEl.value || null) : null,
...
function deletePurchase(purchaseId){
  ...
  if(!pu) return;
  if(!confirm(`¿Quitar la compra a "${pu.supplier}"?`)) return;
