...
/* ===================== COMPRAS ===================== */
// Orden de las listas de compras: primero las que siguen abiertas (más recientes
// arriba) y, al fondo, las RECIBIDAS (compras ya resueltas), también por fecha.
function comparePurchases(a, b){
  const ra = a.status==='RECIBIDA' ? 1 : 0, rb = b.status==='RECIBIDA' ? 1 : 0;
  return (ra - rb) || (b.date||'').localeCompare(a.date||'');
}
function purchaseRowClass(pu){ return pu.status==='RECIBIDA' ? 'pu-received' : ''; }
function purchaseSupplierCell(pu){
  return (pu.status==='RECIBIDA' ? '<span class="pu-check" title="Compra recibida">✓</span>' : '') + esc(pu.supplier||'—');
}
function purchasesForProject(projectId){
  return ensurePurchases().filter(pu=>pu.projectId===projectId).sort(comparePurchases);
}
...
function purchasesSectionHtml(p){
  ...
  const rows = list.map(pu=>`<tr class="${purchaseRowClass(pu)}">
    <td>${purchaseSupplierCell(pu)}</td>
...
function renderPurchases(){
  ...
  let list = DB.purchases.slice().sort(comparePurchases);
  ...
  const rows = list.map(pu=>{
    const proj = pu.projectId ? DB.projects.find(x=>x.id===pu.projectId) : null;
    return `<tr class="${purchaseRowClass(pu)}">
      <td>${purchaseSupplierCell(pu)}</td>
      <td>${esc(purchaseMaterialName(pu))}</td>
