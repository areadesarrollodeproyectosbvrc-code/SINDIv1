...
function exportProjectsExcel(ids){
  ...
  const rows = list.map(p=>{
    ...
    return {
      ...
      'Etapa': p.stage||'', 'Áreas asociadas': projectAreas(p).join(', '), 'Estado': p.status, 'Prioridad': p.priority||'',
...
function printTable(title, columns, rows, targetSelector){
  ...
  target.innerHTML = `
    <div class="print-header">
      <div class="print-header-brand"><img class="print-header-logo" src="icons/logo-bomberos.png" alt="Logo Bomberos"><div><h1>SINDI</h1><p>${esc(ORG_NAME)}</p></div></div>
...
function printProjects(ids){
  const list = DB.projects.filter(p=>ids.includes(p.id));
  const rows = list.map(p=>[p.code, p.name, stationNameById(p.stationId), p.stage||'—', projectAreas(p).join(', ')||'—', p.status, p.priority||'', (p.progress||0)+'%', p.dueDate||'']);
  printTable('Proyectos', ['Código','Nombre','Dependencia','Etapa','Áreas','Estado','Prioridad','Avance','Fecha prevista'], rows, '#print-projects');
}
...
function printSpace(spaceId){
  ...
  target.innerHTML = `
    <div class="print-header">
      <div class="print-header-brand"><img class="print-header-logo" src="icons/logo-bomberos.png" alt="Logo Bomberos"><div><h1>SINDI</h1><p>${esc(ORG_NAME)} — Sistema Integral de Necesidades y Diagnóstico de Infraestructura</p></div></div>
...
/* ---- INFORME PDF DE PROYECTO (para elevar a Jefatura el avance) ----
   Impresión ÚNICA y completa del proyecto: datos generales, descripción,
   observaciones, necesidades, tareas, plan de trabajo (hitos) con sus
   curvas de avance dibujadas como gráfico, cómputo de materiales y compras.
   Mismo formato A4 / mismos bloques (.print-header, .print-section,
   .print-kv, .print-footer) que el informe de relevamiento. Las curvas de
   avance no tienen una impresión aparte: forman parte de este informe. */
function printProject(projectId){
  ...
  target.innerHTML = `
    <div class="print-header">
      <div class="print-header-brand"><img class="print-header-logo" src="icons/logo-bomberos.png" alt="Logo Bomberos"><div><h1>SINDI</h1><p>${esc(ORG_NAME)} — Informe de avance de proyecto</p></div></div>
      <div class="print-meta">Generado: ${today}<br>Por: ${esc(generatedBy)}</div>
    </div>
    <div class="print-section">
      ...
      <table class="print-kv">
        ...
        <tr><td>Etapa de reestructuración</td><td>${p.stage ? esc(p.stage) : '—'}</td></tr>
        <tr><td>Áreas asociadas</td><td>${projectAreas(p).length ? esc(projectAreas(p).join(', ')) : '—'}</td></tr>
...
/* ---- SOLICITUD DE COMPRA (PDF) ----
   Solicitud de compra con los datos del proyecto de origen,
   los materiales pedidos y, si están cargados, los presupuestos
   adjuntos (imagen o PDF) como respaldo. */
function printPurchase(purchaseId){
  ...
  const today = new Date().toLocaleDateString('es-AR');
  const generatedBy = session ? session.name : 'Sistema';
  const target = document.querySelector('#print-purchase');
  if(!target) return;

  const mats = purchaseMaterialsDetailed(pu);
  const matRows = mats.length ? mats.map(m=>`<tr>
      <td>${esc(m.type)}</td><td>${esc(String(m.qty??0))}</td><td>${esc(m.unit||'—')}</td>
      <td>${esc(m.brand||'—')}</td><td>${money(m.price)}</td><td>${money(materialSubtotal(m))}</td>
    </tr>`).join('')
    : `<tr><td colspan="6">${esc(pu.materialText || 'Sin materiales del cómputo vinculados a esta solicitud.')}</td></tr>`;
  const matTotal = mats.reduce((acc,m)=>acc+materialSubtotal(m), 0);

  const attachments = DB.files.filter(f=>f.storeKey==='purchase:'+pu.id);
  const isImage = f => /\.(jpe?g|png)$/i.test(f.name||'');
  const images = attachments.filter(isImage);
  const pdfs = attachments.filter(f=>!isImage(f));
  const attachmentsHtml = attachments.length===0
    ? `<p class="print-obs">Sin presupuestos ni comprobantes adjuntos.</p>`
    : `
      ${images.length ? printPhotoGallery(images) : ''}
      ${pdfs.length ? `<ul style="margin:8px 0 0;padding-left:18px;font-size:12px;">
          ${pdfs.map(f=>`<li>Presupuesto adjunto (PDF): ${esc(f.name)}${f.uploadedAt?' · cargado el '+new Date(f.uploadedAt).toLocaleDateString('es-AR'):''}</li>`).join('')}
        </ul><p class="print-obs" style="margin-top:4px;">Los archivos PDF adjuntos no se incrustan en esta hoja: se listan acá y quedan disponibles en la carpeta de Drive / gestor de archivos de la compra.</p>` : ''}`;

  target.innerHTML = `
    <div class="print-header">
      <div class="print-header-brand"><img class="print-header-logo" src="icons/logo-bomberos.png" alt="Logo Bomberos"><div><h1>SINDI</h1><p>${esc(ORG_NAME)} — Solicitud de compra</p></div></div>
      <div class="print-meta">Generado: ${today}<br>Por: ${esc(generatedBy)}</div>
    </div>
    <div class="print-section">
    ...
    ${p ? `
    <div class="print-section">
      <h3>Proyecto de origen</h3>
      <table class="print-kv">
        <tr><td>Proyecto</td><td>${esc(p.name)} (${esc(p.code)})</td></tr>
        <tr><td>Dependencia</td><td>${esc(stationNameById(p.stationId))}</td></tr>
        <tr><td>Estado del proyecto</td><td>${esc(p.status)}</td></tr>
        <tr><td>Prioridad</td><td>${esc(p.priority||'—')}</td></tr>
        <tr><td>Responsable del proyecto</td><td>${esc(p.responsible||'—')}</td></tr>
        <tr><td>Fecha prevista del proyecto</td><td>${esc(p.dueDate||'—')}</td></tr>
      </table>
    </div>` : `
    <div class="print-section"><p class="print-obs">Solicitud de compra general, sin proyecto asociado.</p></div>`}
    <div class="print-section">
      <h3>Datos de la solicitud</h3>
      <table class="print-kv">
        <tr><td>Proveedor</td><td>${esc(pu.supplier||'—')}</td></tr>
        <tr><td>Autorización de compra</td><td>${esc(pu.authorization||'—')}</td></tr>
        <tr><td>Fecha de la solicitud</td><td>${esc(pu.date||'—')}</td></tr>
        <tr><td>Estado</td><td>${esc(purchaseStatusLabel(pu.status))}</td></tr>
        <tr><td>Responsable del área</td><td>${esc(pu.responsible||'—')}</td></tr>
      </table>
    </div>
    <div class="print-section">
      <h3>Materiales solicitados</h3>
      <table>
        <thead><tr><th>Tipo</th><th>Cant.</th><th>Unidad</th><th>Marca</th><th>Precio ref.</th><th>Subtotal</th></tr></thead>
        <tbody>${matRows}</tbody>
        ${mats.length ? `<tfoot><tr><td colspan="4" style="text-align:right;font-weight:700;">Total estimado</td><td colspan="2" style="font-weight:700;">${money(matTotal)}</td></tr></tfoot>` : ''}
      </table>
    </div>
    <div class="print-section">
      <h3>Observaciones</h3>
      <p class="print-obs">${esc(pu.notes||'Sin observaciones cargadas.')}</p>
    </div>
    <div class="print-section">
      <h3>Presupuestos / comprobantes adjuntos</h3>
      ${attachmentsHtml}
    </div>
    <div class="print-footer">SINDI — Solicitud de compra · ${esc(pu.supplier||'')} · Generado ${today}</div>`;
  target.style.display = 'block';
  waitForImages(target, 6000).then(()=>{
    window.print();
    setTimeout(()=>{ target.style.display='none'; target.innerHTML=''; }, 500);
