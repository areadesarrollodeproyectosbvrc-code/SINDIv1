...
// Estados que se muestran en la lista principal de Proyectos (activos).
// Los FINALIZADO se ven dentro de la misma sección, desde el botón
// "COMPLETADOS", y no aparecen como opción de filtro de los activos.
const ACTIVE_PROJECT_STATUSES = PROJECT_STATUSES.filter(st=>st!=='FINALIZADO');

/* ---- ÁREAS ASOCIADAS ----
   Un proyecto puede pertenecer a varias áreas de la institución (se marcan
   en la ficha). Se guardan como lista de nombres en project.areas y sirven
   para filtrar la lista de proyectos. */
const PROJECT_AREAS = ['Sanidad','Mantenimiento','Protocolo y Ceremonial','Vestuario','USAR','Jefatura','Datos','Recursos Humanos','Conducción','Capacitación','Primer Nivel'];
function projectAreas(p){ return Array.isArray(p.areas) ? p.areas.filter(a=>PROJECT_AREAS.includes(a)) : []; }
function projectAreasHtml(p){
  const a = projectAreas(p);
  return a.length ? `<div class="project-areas">${a.map(x=>`<span>${esc(x)}</span>`).join('')}</div>` : '';
}
function toggleProjectArea(projectId, area, checked){
  const p = DB.projects.find(x=>x.id===projectId);
  if(!p || !PROJECT_AREAS.includes(area)) return;
  const set = new Set(projectAreas(p));
  if(checked) set.add(area); else set.delete(area);
  p.areas = PROJECT_AREAS.filter(a=>set.has(a)); // siempre en el mismo orden
  logAudit('UPDATE','Project', `${p.code} (áreas)`);
  persist();
}

/* Filtros de la lista de proyectos (se combinan entre sí). */
const PROGRESS_RANGES = [
  { key:'0',      label:'Sin avance (0%)', test:v=>v===0 },
  { key:'1-25',   label:'1% a 25%',        test:v=>v>=1 && v<=25 },
  { key:'26-50',  label:'26% a 50%',       test:v=>v>=26 && v<=50 },
  { key:'51-75',  label:'51% a 75%',       test:v=>v>=51 && v<=75 },
  { key:'76-99',  label:'76% a 99%',       test:v=>v>=76 && v<=99 },
  { key:'100',    label:'Completo (100%)', test:v=>v>=100 },
];
function projectFilters(){
  return {
    area: renderProjects._filterArea || '',
    progress: renderProjects._filterProgress || '',
    stage: renderProjects._filterStage || '',
    priority: renderProjects._filterPriority || '',
  };
}
function projectPassesFilters(p, f){
  if(f.area && !projectAreas(p).includes(f.area)) return false;
  if(f.progress){
    const r = PROGRESS_RANGES.find(x=>x.key===f.progress);
    if(r && !r.test(parseInt(p.progress)||0)) return false;
  }
  if(f.stage){
    if(f.stage==='NONE'){ if(p.stage) return false; }
    else if(String(p.stage||'') !== f.stage) return false;
  }
  if(f.priority && p.priority !== f.priority) return false;
  return true;
}
function clearProjectFilters(){
  renderProjects._filterStatus = '';
  renderProjects._filterArea = ''; renderProjects._filterProgress = '';
  renderProjects._filterStage = ''; renderProjects._filterPriority = '';
  render();
}

function projectStageCell(p){ return p.stage ? esc(p.stage) : '—'; }
function toggleCompletedProjects(){
  renderProjects._showCompleted = !renderProjects._showCompleted;
  render();
}

// Listado de proyectos finalizados (mismos datos y misma ficha que los activos).
function completedProjectsCardHtml(list){
  function completedRow(p){
    return `<tr class="tap">
      <td class="mono" onclick="openProjectDetail('${p.id}')">${esc(p.code)}</td>
      <td onclick="openProjectDetail('${p.id}')">${esc(p.name)}${projectAreasHtml(p)}</td>
      ...
      <td onclick="openProjectDetail('${p.id}')">${projectStageCell(p)}</td>
  ...
  return `<div class="card completed-projects" style="margin-bottom:16px;">
    <div class="station-group-head" style="align-items:center;flex-wrap:wrap;">
      <h3 style="font-size:13.5px;margin:0;">Proyectos completados <span style="font-weight:400;color:var(--ink-soft);font-size:12px;">(${list.length})</span></h3>
      ${list.length ? `<div style="display:flex;gap:6px;flex-wrap:wrap;">
        <button class="btn btn-ghost btn-sm" onclick='exportProjectsExcel(${JSON.stringify(list.map(p=>p.id))})'>Exportar a Excel</button>
        <button class="btn btn-ghost btn-sm" onclick='printProjects(${JSON.stringify(list.map(p=>p.id))})'>Imprimir / PDF (listado)</button>
      </div>` : ''}
    </div>
    ${list.length===0 ? '<div class="empty">Todavía no hay proyectos completados.</div>' :
    `<table><thead><tr><th>Código</th><th>Nombre</th><th>Dependencia</th><th>Espacio relevado</th><th>Etapa</th><th>Prioridad</th><th>Avance</th><th>Fecha prevista</th><th>Responsable</th><th>Inversión</th><th></th></tr></thead><tbody>${list.map(completedRow).join('')}</tbody></table>`}
  </div>`;
}
...
function renderProjects(){
  ...
  const showCompleted = !!renderProjects._showCompleted;
  const f = projectFilters();
  const anyFilter = !!(filterStatus || f.area || f.progress || f.stage || f.priority);
  const completed = DB.projects.filter(p=>p.status==='FINALIZADO' && projectPassesFilters(p,f)).slice().sort((a,b)=> (b.createdAt||'').localeCompare(a.createdAt||''));
  let list = DB.projects.filter(p=>p.status!=='FINALIZADO' && projectPassesFilters(p,f)).slice().sort((a,b)=> (b.createdAt||'').localeCompare(a.createdAt||''));
  if(filterStatus) list = list.filter(p=>p.status===filterStatus);
  const stageOptions = [...new Set(DB.projects.map(p=>p.stage).filter(Boolean).map(String))].sort((a,b)=>Number(a)-Number(b));
  ...
  function stationGroupTable(label, items){
    return `<div class="card station-group" style="margin-bottom:16px;">
      ...
      `<table><thead><tr><th>Código</th><th>Nombre</th><th>Espacio relevado</th><th>Etapa</th><th>Estado</th><th>Prioridad</th><th>Avance</th><th>Fecha prevista</th><th>Responsable</th><th>Inversión</th></tr></thead><tbody>${items.map(p=>projectRow(p,false)).join('')}</tbody></table>`}
    </div>`;
  }
  function flatTable(items){
    return `<div class="card">
      ${items.length===0 ? '<div class="empty">No hay proyectos para mostrar.</div>' :
      `<table><thead><tr><th>Código</th><th>Nombre</th><th>Dependencia</th><th>Espacio relevado</th><th>Etapa</th><th>Estado</th><th>Prioridad</th><th>Avance</th><th>Fecha prevista</th><th>Responsable</th><th>Inversión</th></tr></thead><tbody>${items.map(p=>projectRow(p,true)).join('')}</tbody></table>`}
    </div>`;
  }
  ...
  if(orderBy==='PRIORIDAD' || orderBy==='AVANCE'){
    ...
    bodyHtml = list.length===0 ? `<div class="card"><div class="empty">${DB.projects.length ? (anyFilter ? 'Ningún proyecto activo coincide con los filtros.' : 'No hay proyectos activos para mostrar.') : 'Todavía no hay proyectos. Se crean convirtiendo una necesidad o un pedido.'}</div></div>` : `
      ${groups.map(g=>stationGroupTable(g.label, g.items)).join('')}
      ${orphanItems.length>0 ? stationGroupTable('Sin dependencia asignado', orphanItems) : ''}
    `;
  }
  ...
  const html = `
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <select class="filter-select" onchange="renderProjects._filterArea=this.value; render();">
          <option value="">Todas las áreas</option>
          ${PROJECT_AREAS.map(a=>`<option value="${esc(a)}" ${f.area===a?'selected':''}>${esc(a)}</option>`).join('')}
        </select>
        <select class="filter-select" onchange="renderProjects._filterProgress=this.value; render();">
          <option value="">Todos los avances</option>
          ${PROGRESS_RANGES.map(r=>`<option value="${r.key}" ${f.progress===r.key?'selected':''}>${r.label}</option>`).join('')}
        </select>
        <select class="filter-select" onchange="renderProjects._filterStage=this.value; render();">
          <option value="">Todas las etapas</option>
          ${stageOptions.map(x=>`<option value="${esc(x)}" ${f.stage===x?'selected':''}>Etapa ${esc(x)}</option>`).join('')}
          <option value="NONE" ${f.stage==='NONE'?'selected':''}>Sin etapa</option>
        </select>
        <select class="filter-select" onchange="renderProjects._filterPriority=this.value; render();">
          <option value="">Todas las prioridades</option>
          ${['ALTA','MEDIA','BAJA'].map(x=>`<option value="${x}" ${f.priority===x?'selected':''}>${x.charAt(0)+x.slice(1).toLowerCase()}</option>`).join('')}
        </select>
        ...
        ${anyFilter ? `<button class="btn btn-ghost" onclick="clearProjectFilters()">Limpiar filtros</button>` : ''}
        <button class="btn ${showCompleted?'btn-primary':'btn-ghost'}" aria-expanded="${showCompleted}" onclick="toggleCompletedProjects()">COMPLETADOS (${completed.length}) ${showCompleted?'▴':'▾'}</button>
      </div>
      <div class="toolbar-right">
    ...
    ${showCompleted ? completedProjectsCardHtml(completed) : ''}
    ${bodyHtml}
...
function renderProjectDetail(){
  ...
  const html = `
    <div class="link-tab" style="margin-bottom:10px;" onclick="projectDetailState.projectId=null; render();">&larr; Todos los proyectos</div>
    ...
    <div class="card" style="margin-bottom:16px;">
      <div class="row-name-state-stage">
        ...
        <label class="field light"><span>Etapa de reestructuración</span><input data-pf="stage" type="number" min="1" step="1" placeholder="Ej: 1" value="${p.stage||''}"></label>
      </div>
      <div class="row3">
        <label class="field light"><span>Prioridad</span>
      ...
      <div class="field light">
        ...
        <div class="area-checks">
          ${PROJECT_AREAS.map(a=>`<label class="area-check"><input type="checkbox" ${projectAreas(p).includes(a)?'checked':''} onchange="toggleProjectArea('${p.id}', this.dataset.area, this.checked)" data-area="${esc(a)}"><span>${esc(a)}</span></label>`).join('')}
        </div>
      </div>
  ...
  document.querySelectorAll('[data-pf]').forEach(inp=>{
    inp.addEventListener('change', ()=>{
      ...
      p[f] = (f==='progress') ? (parseInt(inp.value)||0) : (f==='investment' ? (parseFloat(inp.value)||0) : (f==='stage' ? (parseInt(inp.value)>0 ? parseInt(inp.value) : '') : inp.value));
      logAudit('UPDATE','Project', p.code);
      persist();
      if(f==='investment') render(); // refresca el "$X millones" formateado
      if(f==='stage' && p.stage==='') inp.value = '';
    });
  });
  wireFileManagerEvents({ folderPath, storeKey: 'project:'+p.id+':'+activeCat });
  const renderInput = document.getElementById('project-render-input');
  if(renderInput){ renderInput.addEventListener('change', ()=> uploadProjectRender(renderInput, p.id)); }
}

/* ---- RENDER DEL PROYECTO ----
   Una sola imagen (JPG/PNG/WebP) que se muestra arriba de la ficha, sin
   nombre de archivo. Vive en R2 bajo PROYECTOS/<proyecto>/RENDER y su
   metadato en project.render. Al reemplazarla o quitarla, la anterior
   también se borra del servidor. */
const PROJECT_RENDER_TYPES = ['image/jpeg','image/png','image/webp'];
function projectRenderHtml(p){
  const input = `<input id="project-render-input" type="file" accept="image/jpeg,image/png,image/webp" style="display:none;">`;
  if(!p.render || !p.render.url){
    return `<label class="dropzone project-render-empty">+ Subir render del proyecto (JPG / PNG)${input}</label>`;
  }
  return `<div class="project-render">
    <a href="${esc(p.render.viewUrl || p.render.url)}" target="_blank" rel="noopener"><img class="project-render-img" src="${esc(p.render.url)}" alt="Render de ${esc(p.name)}"></a>
    <div class="project-render-actions">
      <label class="btn btn-ghost btn-sm">Reemplazar${input}</label>
      <button class="btn btn-ghost btn-sm" onclick="removeProjectRender('${p.id}')">Quitar</button>
    </div>
  </div>`;
}
async function uploadProjectRender(inputEl, projectId){
  const p = DB.projects.find(x=>x.id===projectId);
  const file = inputEl.files && inputEl.files[0];
  if(!p || !file) return;
  if(!PROJECT_RENDER_TYPES.includes(file.type)){
    toast('El render debe ser una imagen JPG, PNG o WebP');
    inputEl.value = '';
    return;
  }
  toast('Subiendo render...');
  try{
    const result = await driveUploadFile({ file, folderPath: ['PROYECTOS', projectFolderSegment(p), 'RENDER'], description: `Render de ${p.code}` });
    const old = p.render;
    p.render = { name: file.name, url: result.url || '', viewUrl: result.viewUrl || '', uploadedAt: new Date().toISOString(), storedIn:'r2' };
    if(old) deleteStoredFiles([old]).catch(e=>console.error('[SINDI] No se pudo borrar el render anterior:', e));
  }catch(err){
    toast('Error al subir el render: ' + err.message, true);
    inputEl.value = '';
    return;
  }
  logAudit('UPLOAD','Project render', p.code);
  persist(); render();
}
async function removeProjectRender(projectId){
  const p = DB.projects.find(x=>x.id===projectId);
  if(!p || !p.render) return;
  if(!confirm('¿Quitar el render del proyecto? Se borra también del servidor y no se puede recuperar.')) return;
  try{ await deleteStoredFiles([p.render]); }
  catch(err){ toast('No se pudo borrar el render del servidor: ' + err.message, true); return; }
  p.render = null;
  logAudit('DELETE','Project render', p.code);
  persist(); render();
}
...
async function deleteProject(id){
  ...
  // Comprobantes de las compras del proyecto: las compras se borran con el proyecto,
  // así que sus archivos también tienen que salir del servidor.
  const purchaseFiles = DB.files.filter(f=>linkedPurchases.some(pu=>f.storeKey==='purchase:'+pu.id));
  const serverFiles = [...files, ...purchaseFiles, ...(p.render ? [p.render] : [])]; // archivos + comprobantes + render
  const extra = serverFiles.length ? ` Se borran también sus ${serverFiles.length} archivo(s) del servidor.` : '';
  if(!confirm(`¿Eliminar el proyecto "${p.code} — ${p.name}"? Esta acción no se puede deshacer.${extra}`)) return;
  try{
    if(serverFiles.length) await deleteStoredFiles(serverFiles);
  }catch(err){
    toast('No se pudieron borrar los archivos del servidor: ' + err.message, true);
    return;
  }
  DB.files = DB.files.filter(f=>!files.includes(f) && !purchaseFiles.includes(f));
