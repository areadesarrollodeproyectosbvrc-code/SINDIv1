...
function openNeedModal(spaceId){
  modal = { type:'need' };
  const fixedSpace = spaceId ? DB.spaces.find(x=>x.id===spaceId) : null;
  const fixedStation = fixedSpace ? DB.stations.find(x=>x.id===fixedSpace.stationId) : null;
  renderModal(`
    <h2>Nueva necesidad</h2>
    ${fixedSpace ? `
      <div class="field light"><span>Dependencia</span><div style="padding:9px 0;font-size:13.5px;">${esc(fixedStation?.name||'—')}</div></div>
      <div class="field light"><span>Espacio</span><div style="padding:9px 0;font-size:13.5px;">${esc(fixedSpace.name)}</div></div>
    ` : `
      <div class="row2">
    ...
    `}
    <label class="field light"><span>Título</span><input id="m-need-title" type="text" placeholder="Ej: Filtraciones en cubierta del vestuario"></label>
    <label class="field light"><span>Descripción</span><textarea id="m-need-desc" rows="3"></textarea></label>
    <div class="row2">
      <label class="field light"><span>Prioridad</span>
        <select id="m-need-priority"><option value="BAJA">Baja</option><option value="MEDIA">Media</option><option value="ALTA">Alta</option></select>
      </label>
      <label class="field light"><span>Categoría</span>
        <select id="m-need-cat">
          <option value="">Sin categorizar</option>
          ${NEED_CATEGORIES.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}
        </select>
      </label>
    </div>
    <label class="field light"><span>Responsable (opcional)</span><input id="m-need-resp" type="text"></label>
    <div class="modal-actions">
      <button class="btn btn-ghost" onclick="closeModal()">Cancelar</button>
      <button class="btn btn-primary" onclick="saveNeed('${spaceId||''}')">Agregar</button>
    </div>`);
}
...
function renderNeeds(){
  ...
  function needRow(n){
    ...
    return `<tr>
      ...
      <td>${n.status!=='CONVERTIDO_EN_PROYECTO' ? `<span class="link-tab" onclick="convertNeedToProject('${n.id}')">Convertir en proyecto</span>` : `<span class="link-tab" onclick="goRoute('projects'); openProjectDetail('${n.projectId}')">Ver proyecto</span>`}</td>
    </tr>`;
  }
...
function openProjectDetail(id){ projectDetailState = { projectId: id, returnRoute: route }; render(); }
    ...
    <div class="card" style="margin-bottom:16px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;">
        <div>
          ...
          ${p.driveLink ? `<a href="${esc(p.driveLink)}" target="_blank" rel="noopener" class="btn btn-ghost btn-sm" style="display:inline-flex;align-items:center;gap:6px;margin-top:8px;">📁 Abrir carpeta de Drive del proyecto →</a>` : ''}
        </div>
        <button class="btn btn-ghost btn-sm" style="flex-shrink:0;" onclick="printProject('${p.id}')">Generar informe PDF</button>
      </div>
    </div>
    <div class="card" style="margin-bottom:16px;">
      <label class="field light"><span>Link de la carpeta de Drive del proyecto</span>
        <input data-pf="driveLink" type="url" placeholder="Pegá acá el link de la carpeta de Drive (lo puede cargar o cambiar cualquier usuario)" value="${esc(p.driveLink||'')}">
      </label>
    </div>
    <div class="card" style="margin-bottom:16px;">
      <div class="row2">
