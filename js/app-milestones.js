...
function milestoneCurveHtml(p){
  ...
  const W = 620, H = 190, padL = 34, padR = 12, padT = 12, padB = 26;
  ...
  const dots = actualPts.slice(1).map(pt=>`<circle cx="${pt[0].toFixed(1)}" cy="${pt[1].toFixed(1)}" r="3" fill="#a31f1f"></circle>`).join('');

  return `
    <div style="margin-top:12px;">
      <div style="display:flex;gap:14px;font-size:10.5px;color:var(--ink-soft);margin-bottom:4px;">
        <span><span style="display:inline-block;width:14px;height:2px;background:#4f7fc0;vertical-align:middle;"></span> Planificado</span>
        <span><span style="display:inline-block;width:14px;height:2px;background:#a31f1f;vertical-align:middle;"></span> Real</span>
      </div>
      <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;">
        ${gridLines}
        ${xLabels}
        ${plannedPts.length ? `<path d="${pathOf(plannedPts)}" fill="none" stroke="#4f7fc0" stroke-width="2" stroke-dasharray="5 3"></path>` : ''}
        ${actualPts.length ? `<path d="${pathOf(actualPts)}" fill="none" stroke="#a31f1f" stroke-width="2.5"></path>` : ''}
        ${dots}
      </svg>
    </div>`;
}
...
/* ---- Bloque del plan de trabajo para el informe PDF del proyecto.
   Solo texto (sin gráficos ni imágenes), coherente con el resto del
   informe que se eleva a Jefatura. ---- */
