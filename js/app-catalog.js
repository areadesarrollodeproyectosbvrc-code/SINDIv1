...
function saveStation(id){
  ...
  if(id){
    ...
    DB.stations.push({ id: uid(), name, address, active:true, planImage:null, levelPlans:{} });
...
async function deactivateStation(id){
  const s = DB.stations.find(x=>x.id===id);
  if(!s) return;
  const levelPlanList = s.levelPlans ? Object.values(s.levelPlans).filter(Boolean) : [];
  const planNote = (s.planImage || levelPlanList.length>0) ? ' Se borran también sus fotos y planos del servidor.' : '';
  if(!confirm(`¿Dar de baja \"${s.name}\"?${planNote}`)) return;
  try{
    const toDelete = [...(s.planImage ? [s.planImage] : []), ...levelPlanList];
    if(toDelete.length) await deleteStoredFiles(toDelete);
  }catch(err){
    toast('No se pudieron borrar las fotos y planos del servidor: ' + err.message, true);
    return;
  }
...
function renderStationDetail(){
  ...
  const html = `
    ...
    <div class="card" style="margin-bottom:16px;">
      <h3 style="font-size:13.5px;margin:0 0 10px;">Foto de fachada</h3>
      <label class="btn btn-ghost btn-sm" style="display:inline-block;">
        ${s.planImage ? 'Reemplazar foto de fachada' : '+ Subir foto de fachada (JPG/PNG)'}
        <input id="station-plan-input" type="file" accept="image/*" style="display:none;">
      </label>
      ${s.planImage ? `<button class="btn btn-ghost btn-sm" onclick="removeStationPlan('${s.id}')">Quitar foto</button>` : ''}
      <div style="margin-top:4px;"></div>
      ${s.planImage && s.planImage.url ? `<div style="margin-top:12px;"><a href="${esc(s.planImage.viewUrl || s.planImage.url)}" target="_blank"><img src="${esc(s.planImage.url)}" alt="Foto de fachada de ${esc(s.name)}" class="station-facade-img"></a></div>` : '<div class="empty">Todavía no se cargó la foto de fachada de esta dependencia.</div>'}
...
async function uploadStationPlan(inputEl, stationId){
  ...
  if(!s || !file) return;
  toast('Subiendo foto de fachada...');
  try{
    const result = await driveUploadFile({ file, folderPath: ['CUARTELES', stationFolderSegment(s)], description: `Foto de fachada de ${s.name}` });
    const oldPlan = s.planImage;
    s.planImage = { name: file.name, url: result.url || '', viewUrl: result.viewUrl || '', uploadedAt: new Date().toISOString(), storedIn:'r2' };
    // La foto anterior queda reemplazada: también se borra del servidor.
    if(oldPlan) deleteStoredFiles([oldPlan]).catch(e=>console.error('[SINDI] No se pudo borrar la foto de fachada anterior:', e));
  }catch(err){
    toast('Error al subir la foto de fachada: ' + err.message, true); return;
  }
  logAudit('UPLOAD','Station facade photo', s.name);
  persist(); render();
}

async function removeStationPlan(stationId){
  const s = DB.stations.find(x=>x.id===stationId);
  if(!s || !s.planImage) return;
  if(!confirm('¿Quitar la foto de fachada? Se borra también del servidor y no se puede recuperar.')) return;
  try{ await deleteStoredFiles([s.planImage]); }
  catch(err){ toast('No se pudo borrar la foto del servidor: ' + err.message, true); return; }
  s.planImage = null;
  logAudit('DELETE','Station facade photo', s.name);
  persist(); render();
}

/* ---- PLANO POR NIVEL (Planta Baja / Planta Alta / Terraza / Patio / Otro) ----
   Cada nivel de cada dependencia puede tener su propia imagen de planta
   arquitectónica con los códigos de espacio, para corroborar a qué lugar
   relevado pertenece cada relevamiento. Se muestra arriba de la lista de
   relevamientos de ese nivel. */
function ensureStationLevelPlans(s){
  if(!s.levelPlans || typeof s.levelPlans !== 'object') s.levelPlans = {};
  SPACE_LEVELS.forEach(l=>{ if(!(l.key in s.levelPlans)) s.levelPlans[l.key] = null; });
}
function levelFolderSegment(levelKey){ return spaceLevelLabel(levelKey); }
async function uploadLevelPlan(inputEl, stationId, levelKey){
  const s = DB.stations.find(x=>x.id===stationId);
  const file = inputEl.files && inputEl.files[0];
  if(!s || !file) return;
  ensureStationLevelPlans(s);
  toast('Subiendo plano...');
  try{
    const result = await driveUploadFile({ file, folderPath: ['CUARTELES', stationFolderSegment(s), levelFolderSegment(levelKey)], description: `Plano de ${spaceLevelLabel(levelKey)} — ${s.name}` });
    const oldPlan = s.levelPlans[levelKey];
    s.levelPlans[levelKey] = { name: file.name, url: result.url || '', viewUrl: result.viewUrl || '', uploadedAt: new Date().toISOString(), storedIn:'r2' };
    // El plano anterior queda reemplazado: también se borra del servidor.
    if(oldPlan) deleteStoredFiles([oldPlan]).catch(e=>console.error('[SINDI] No se pudo borrar el plano anterior:', e));
  }catch(err){
    toast('Error al subir el plano: ' + err.message, true); return;
  }
  logAudit('UPLOAD','Level plan', `${s.name} · ${spaceLevelLabel(levelKey)}`);
  persist(); render();
}
async function removeLevelPlan(stationId, levelKey){
  const s = DB.stations.find(x=>x.id===stationId);
  if(!s || !s.levelPlans || !s.levelPlans[levelKey]) return;
  if(!confirm('¿Quitar el plano de este nivel? Se borra también del servidor y no se puede recuperar.')) return;
  try{ await deleteStoredFiles([s.levelPlans[levelKey]]); }
  catch(err){ toast('No se pudo borrar el plano del servidor: ' + err.message, true); return; }
  s.levelPlans[levelKey] = null;
  logAudit('DELETE','Level plan', `${s.name} · ${spaceLevelLabel(levelKey)}`);
  persist(); render();
}
...
function renderSpaces(){
  ...
  ensureStationLevelPlans(station);
  ...
  const levelPlan = station.levelPlans[level];
  const html = `
    ...
    <div class="card station-group">
      <div class="station-group-head" style="align-items:flex-start; flex-wrap:wrap;">
        <div style="display:flex; align-items:center; gap:12px; min-width:0;">
          ${levelPlan && levelPlan.url
            ? `<a href="${esc(levelPlan.viewUrl || levelPlan.url)}" target="_blank"><img class="level-plan-thumb" src="${esc(levelPlan.url)}" alt="Plano de ${esc(spaceLevelLabel(level))} — ${esc(station.name)}"></a>`
            : `<div class="level-plan-thumb-ph"></div>`}
        ...
        <div style="display:flex; gap:6px; flex-wrap:wrap; flex:0 0 auto;">
          <label class="btn btn-ghost btn-sm" style="display:inline-block;">
            ${levelPlan ? 'Reemplazar plano' : '+ Subir plano (JPG/PNG)'}
            <input id="level-plan-input" type="file" accept="image/*" style="display:none;">
          </label>
          ${levelPlan ? `<button class="btn btn-ghost btn-sm" onclick="removeLevelPlan('${station.id}','${level}')">Quitar plano</button>` : ''}
        </div>
      </div>
  ...
  renderShell(html, 'Espacios y relevamientos', 'El plano de este nivel te sirve para corroborar a qué lugar relevado pertenece cada relevamiento.');
  const levelPlanInput = document.getElementById('level-plan-input');
  if(levelPlanInput){ levelPlanInput.addEventListener('change', ()=> uploadLevelPlan(levelPlanInput, station.id, level)); }
}
...
function renderSpaceDetail(){
  ...
  if(detailState.tab==='general'){
    body = `<div class="card">
      <div class="row2">
        <label class="field light"><span>Código (CUE)</span><input data-f="cue" type="text" value="${esc(s.cue)}"></label>
  ...
  const html = `
    ...
    <div class="card" style="margin-bottom:16px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;">
        <div>
          <span id="space-head-cue" class="mono" style="font-size:12px;background:#eeece5;padding:2px 8px;border-radius:6px;">${esc(s.cue)}</span>
          <div id="space-head-name" style="font-size:16px;font-weight:700;margin-top:6px;">${esc(s.name)}</div>
          <div id="space-head-meta" style="font-size:12px;color:var(--ink-soft);">${spaceHeadMetaText(s)}</div>
        </div>
        <div style="display:flex; gap:6px; flex-shrink:0;">
          <button class="btn btn-ghost btn-sm" onclick="printSpace('${s.id}')">Imprimir / PDF</button>
          <button class="btn btn-ghost btn-sm" style="color:var(--danger);" onclick="deleteSpace('${s.id}')">Eliminar relevamiento</button>
        </div>
      </div>
    </div>
...
// Texto de la segunda línea del encabezado de la ficha del espacio.
function spaceHeadMetaText(s){
  const station = DB.stations.find(x=>x.id===s.stationId);
  return `${esc(station?.name||'')} · ${esc(spaceLevelLabel(s.level))} · ${esc(s.surfaceType)} · ${m2(s.surfaceM2)} m²`;
}
// Actualiza el encabezado en el lugar (sin redibujar la ficha), para que un
// cambio de código, nombre o m² se vea al instante sin perder el foco del campo siguiente.
function refreshSpaceHead(s){
  const cue = document.getElementById('space-head-cue');
  const name = document.getElementById('space-head-name');
  const meta = document.getElementById('space-head-meta');
  if(cue) cue.textContent = s.cue;
  if(name) name.textContent = s.name;
  if(meta) meta.innerHTML = spaceHeadMetaText(s);
}
function wireSpaceDetailEvents(s){
  document.querySelectorAll('[data-f]').forEach(inp=>{
    inp.addEventListener('change', ()=>{
      ...
      if(f==='cue'){
        const newCue = inp.value.trim();
        if(!newCue){ toast('El código no puede quedar vacío'); inp.value = s.cue; return; }
        const dup = DB.spaces.some(x=>x.id!==s.id && x.stationId===s.stationId && x.cue.toLowerCase()===newCue.toLowerCase() && x.active!==false);
        if(dup){ toast('Ya existe otro espacio con ese código en esta dependencia'); inp.value = s.cue; return; }
        s.cue = newCue;
        inp.value = newCue;
        logAudit('UPDATE','Space', `${s.cue} — ${s.name} (código)`);
        persist(); refreshSpaceHead(s);
        return;
      }
      if(f==='name'){
        const newName = inp.value.trim();
        if(!newName){ toast('El nombre no puede quedar vacío'); inp.value = s.name; return; }
        s.name = newName;
        inp.value = newName;
        logAudit('UPDATE','Space', `${s.cue} — ${s.name} (nombre)`);
        persist(); refreshSpaceHead(s);
        return;
      }
      if(f==='surfaceM2'){
        const n = parseFloat(String(inp.value).replace(',','.'));
        if(isNaN(n) || n < 0){ toast('Ingresá una superficie válida en m²'); inp.value = s.surfaceM2; return; }
        s.surfaceM2 = n;
        logAudit('UPDATE','Space', `${s.cue} — ${s.name} (m²)`);
        persist(); refreshSpaceHead(s);
        return;
      }
      s[f] = inp.value;
...
async function deleteSpace(id){
  ...
  const photos = [
    ...(s.generalPhotos||[]),
    ...((s.pathologies||[]).flatMap(p=>p.photos||[])),
  ];
  const linkedNeeds = DB.needs.filter(n=>n.spaceId===s.id);
  const extra = photos.length ? ` Se borran también sus ${photos.length} foto(s) del servidor.` : '';
  const needsNote = linkedNeeds.length ? ` Las ${linkedNeeds.length} necesidad(es) asociadas quedarán sin espacio vinculado.` : '';
  if(!confirm(`¿Eliminar el relevamiento "${s.cue} — ${s.name}"? Esta acción no se puede deshacer.${extra}${needsNote}`)) return;
  try{
    if(photos.length) await deleteStoredFiles(photos);
  }catch(err){
    toast('No se pudieron borrar las fotos del servidor: ' + err.message, true);
    return;
  }
  linkedNeeds.forEach(n=>{ n.spaceId = null; });
  DB.spaces = DB.spaces.filter(x=>x.id!==id);
  logAudit('DELETE','Space', `${s.cue} — ${s.name}`);
  persist();
  detailState.spaceId = null;
  render();
        ...
        ${s.generalPhotos.map(ph=>`
          <a href="${esc(ph.viewUrl || ph.url)}" target="_blank" class="photo-card" style="text-decoration:none;color:inherit;">
            ...
            <div class="pc-body">
              ...
              <button type="button" class="btn btn-ghost btn-sm" style="margin-top:6px;" onclick="removeGeneralPhoto('${s.id}','${ph.id}',event)">Quitar</button>
            </div>
          </a>`).join('')}
      </div>`}
  </div>`;
...
async function removePathology(i){
  const s = DB.spaces.find(x=>x.id===detailState.spaceId);
  const p = s && s.pathologies[i];
  if(!p) return;
  const n = (p.photos||[]).length;
  const extra = n ? ` Se borran también sus ${n} foto(s) del servidor.` : '';
  if(!confirm(`¿Quitar la patología \"${p.category}\"?${extra}`)) return;
  try{ await deleteStoredFiles(p.photos||[]); }
  catch(err){ toast('No se pudieron borrar las fotos del servidor: ' + err.message, true); return; }
  const idx = s.pathologies.findIndex(x=>x.id===p.id);
  if(idx>=0) s.pathologies.splice(idx,1);
  logAudit('DELETE','Pathology', `${p.category} en ${s.name}`);
  persist(); render();
}
async function removePathologyPhoto(pathologyId, photoId, ev){
  if(ev){ ev.preventDefault(); ev.stopPropagation(); }
  const s = DB.spaces.find(x=>x.id===detailState.spaceId);
  const p = s && s.pathologies.find(x=>x.id===pathologyId);
  const ph = p && (p.photos||[]).find(x=>x.id===photoId);
  if(!ph) return;
  if(!confirm(`¿Quitar la foto \"${ph.name}\"? Se borra también del servidor y no se puede recuperar.`)) return;
  try{ await deleteStoredFiles([ph]); }
  catch(err){ toast('No se pudo borrar la foto del servidor: ' + err.message, true); return; }
  p.photos = p.photos.filter(x=>x.id!==photoId);
  logAudit('DELETE','Pathology photo', `${ph.name} en ${p.category}`);
  persist(); render();
}
async function removeGeneralPhoto(spaceId, photoId, ev){
  if(ev){ ev.preventDefault(); ev.stopPropagation(); }
  const s = DB.spaces.find(x=>x.id===spaceId);
  const ph = s && (s.generalPhotos||[]).find(x=>x.id===photoId);
  if(!ph) return;
  if(!confirm(`¿Quitar la foto \"${ph.name}\"? Se borra también del servidor y no se puede recuperar.`)) return;
  try{ await deleteStoredFiles([ph]); }
  catch(err){ toast('No se pudo borrar la foto del servidor: ' + err.message, true); return; }
  s.generalPhotos = s.generalPhotos.filter(x=>x.id!==photoId);
  logAudit('DELETE','Space photo', `${ph.name} en ${s.name}`);
  persist(); render();
}
            ...
            ${(p.photos||[]).map(ph=>`
              <a href="${esc(ph.viewUrl || ph.url)}" target="_blank" class="photo-card" style="text-decoration:none;color:inherit;">
                ...
                <div class="pc-body">
                  ...
                  <button type="button" class="btn btn-ghost btn-sm" style="margin-top:6px;" onclick="removePathologyPhoto('${p.id}','${ph.id}',event)">Quitar</button>
                </div>
              </a>`).join('')}
          </div>
        </div>`).join('')}
