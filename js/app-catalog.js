...
async function deactivateStation(id){
  const s = DB.stations.find(x=>x.id===id);
  if(!s) return;
  const plan = s.planImage ? ' Se borra también su plano del servidor.' : '';
  if(!confirm(`¿Dar de baja \"${s.name}\"?${plan}`)) return;
  try{
    if(s.planImage) await deleteStoredFiles([s.planImage]);
  }catch(err){
    toast('No se pudo borrar el plano del servidor: ' + err.message, true);
    return;
  }
...
function renderStationDetail(){
  ...
  const html = `
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      ${s.planImage ? `<button class="btn btn-ghost btn-sm" onclick="removeStationPlan('${s.id}')">Quitar plano</button>` : ''}
...
async function removeStationPlan(stationId){
  ...
  if(!confirm('¿Quitar el plano? Se borra también del servidor y no se puede recuperar.')) return;
  try{ await deleteStoredFiles([s.planImage]); }
  catch(err){ toast('No se pudo borrar el plano del servidor: ' + err.message, true); return; }
  s.planImage = null;
  logAudit('DELETE','Station plan', s.name);
  persist(); render();
}
