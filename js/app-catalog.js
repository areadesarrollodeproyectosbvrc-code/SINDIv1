...
function renderStationDetail(){
  ...
  const html = `
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      ${s.planImage && s.planImage.url ? `<div style="margin-top:12px;"><a href="${esc(s.planImage.viewUrl || s.planImage.url)}" target="_blank"><img src="${esc(s.planImage.url)}" alt="Foto de fachada de ${esc(s.name)}" style="max-width:100%;border:1px solid var(--line);border-radius:8px;"></a></div>` : '<div class="empty">Todavía no se cargó la foto de fachada de esta dependencia.</div>'}
