...
Abrí `https://tu-worker.workers.dev/api/storage` directo en el
navegador. Tiene que responder:
```json
{"ok":true,"message":"SINDI Worker activo. Usar POST para operar."}
```
Si en cambio ves la app (el login de SINDI), revisá que la ruta tenga
exactamente `/api/storage` al final.
